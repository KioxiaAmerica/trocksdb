...
#define DEBLEVEL 0x80  // 1=SST ring ops 2=file ops 4=iterator ops 8=ring pointers 16=deleted_files 32=versions 64=top-level ring ops 128=ring status
...
public:
  ...
  VLogRingFile(std::string pathfname,  // fully qualified pathname
    std::unique_ptr<RandomAccessFile>& fptr  // the file 
  ) :
    queue(nullptr),
    refcount(0),
    filename(pathfname),
...
// scaf obsolete  std::atomic<VLogRingRefFileno> fd_ring_head_fileno_shadow;   // we move head_fileno to reserve space, before the files have been created.  Move the shadow after they are synced
// scaf obsolete   std::atomic <VLogRingRefFileOffset> fd_ring_head_fileoffset;  // the offset that the next write will go to
// scaf obsolete  std::atomic<VLogRingRefFileOffset> fd_ring_head_fileoffset_shadow;  // shadow, as with fileno
...
// See how many files need to be deleted.  Result is a vector of them
