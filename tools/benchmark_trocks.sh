...
function summarize_result {
  ...
  sum_wgb=$( grep "^ Sum" $test_out | tail -2 | head -1 | awk '{ print $9 }' )
  sum_size=$( grep "^ Sum" $test_out | tail -2 | head -1 | awk '{ printf "%.1f", $3 }' )
  wamp=$( grep "^ Sum" $test_out | tail -2 | head -1 | awk '{ print $12 }' )
